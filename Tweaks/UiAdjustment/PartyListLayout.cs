    ...
    public unsafe class PartyListLayout : UiAdjustments.SubTweak {
        ...
        public class ElementConfig {
            ...
            public virtual void Editor(string name, ref bool c, PartyListLayout l = null) {
                c |= ImGui.DragFloat2($"位置##{name}", ref Position);
                c |= ImGui.SliderFloat2($"大小##{name}", ref Scale, 0, 5);
            }
        }
        ...
        public class TextElementConfig : ElementConfig {
            ...
            public override void Editor(string name, ref bool c, PartyListLayout l = null) {
                ...
                c |= ImGui.ColorEdit4($"颜色##{name}", ref Color);
                c |= ImGui.ColorEdit4($"发光##{name}", ref Glow);
            }
        }
        ...
        public class StatusEffectsConfig : ElementConfig {
            ...
            public override void Editor(string name, ref bool c, PartyListLayout l = null) {
                base.Editor(name, ref c, l);
                c |= ImGui.Checkbox($"两列显示##{name}", ref TwoLines);
                if (TwoLines) {
                    ImGui.SameLine();
                    c |= ImGui.Checkbox($"{(Vertical?"优先填充首列": "优先水平填充")}##{name}", ref ReverseFill);
                }
                c |= ImGui.Checkbox($"垂直##{name}", ref Vertical);

                c |= ImGui.SliderFloat2("图标分离", ref Separation, -100, 100);
        ...
        private void ElementConfigEditor(string name, ElementConfig eCfg, ref bool c) {
            ImGui.Text($"{name}: ");
            ImGui.SameLine();
            c |= ImGui.Checkbox($"隐藏##{name}", ref eCfg.Hide);
